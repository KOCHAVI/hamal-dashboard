...
import { Sun, Moon, Save, X, AlertTriangle, Clock, Shield, ChevronRight, ChevronLeft, Calendar, Filter } from 'lucide-react';
...
export const Reservists = () => {
  ...
  const myReservists = useMemo(() => personnel.filter(p => {
    if (!p.isReservist || p.isAdmin) return false;
    if (isAdmin) {
      if (selectedTeamId !== 'all' && p.teamId !== selectedTeamId) return false;
      return true;
    }
    if (activeUser?.isHoT) {
      return p.teamId === activeUser.teamId && p.currentStatus !== PresenceStatus.ABROAD;
    }
    return false;
  ...
  useEffect(() => {
    ...
    const visiblePids = new Set(myReservists.map(p => p.id));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showNotification(`סידור מילואים סונכרן!`, 'success');
      ...
      <div className="p-4 lg:p-8 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 transition-colors">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-indigo-600 dark:bg-indigo-500 rounded-2xl text-white shadow-lg"><Shield size={24} /></div>
            <div>
          ...
          {isAdmin && (
            <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800 p-1.5 rounded-2xl border border-zinc-200 dark:border-zinc-700 mr-0 sm:mr-4">
              ...
              <select className="bg-transparent text-sm font-bold outline-none p-1 min-w-[140px]" value={selectedTeamId} onChange={(e) => { if (!hasChanges || window.confirm('שינויים שלא נשמרו יאבדו. להמשיך?')) setSelectedTeamId(e.target.value); }}>
              ...
              return (
                <div key={dayIdx} className="flex flex-col min-h-[500px] bg-zinc-50/30 dark:bg-zinc-900/10">
                  <div className={clsx("p-3 flex-1 flex flex-col transition-all", dP && "opacity-40 grayscale bg-zinc-200/50 dark:bg-zinc-800/50")}>
                    <div className="flex items-center gap-2 mb-2 text-amber-600 dark:text-amber-500 font-bold text-[9px] bg-amber-50 dark:bg-amber-950/20 p-1.5 rounded-lg border border-amber-100 dark:border-amber-900/30 uppercase"><Sun size={10} /> יום {dP && ' - עבר'}</div>
                    <div className="flex-1 space-y-1">
                      {myReservists.slice().sort((a,b) => {
                        const aS = sDP.includes(a.id), bS = sDP.includes(b.id);
                        if (aS && !bS) return -1; if (!aS && bS) return 1; return a.fullName.localeCompare(b.fullName, 'he');
                      }).map(p => (
                        <button key={p.id} disabled={dP || isSaving} onClick={() => toggleSoldierInSlot(dayKey, p.id, day, 'day')} className={clsx("w-full text-right p-1.5 rounded-lg border transition-all text-[11px] font-bold flex justify-between items-center group", sDP.includes(p.id) ? "bg-indigo-600 dark:bg-indigo-500 border-indigo-700 dark:border-indigo-400 text-white shadow-sm" : "bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:border-indigo-300 dark:hover:border-indigo-500", (dP || isSaving) && "cursor-not-allowed border-transparent shadow-none")}><span className="truncate">{p.fullName}</span>{sDP.includes(p.id) && !dP && <X size={10} className="opacity-50" />}</button>
                      ))}
                    </div>
                  </div>
                  <div className={clsx("p-3 flex-1 flex flex-col border-t border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 transition-all", nP && "opacity-40 grayscale bg-zinc-300/50 dark:bg-zinc-800/50")}>
                    <div className="flex items-center gap-2 mb-2 text-indigo-600 dark:text-indigo-400 font-bold text-[9px] bg-indigo-50 dark:bg-indigo-950/20 p-1.5 rounded-lg border border-indigo-100 dark:border-indigo-900/30 uppercase"><Moon size={10} /> לילה {nP && ' - עבר'}</div>
                    <div className="flex-1 space-y-1">
                      {myReservists.slice().sort((a,b) => {
                        const aS = sNP.includes(a.id), bS = sNP.includes(b.id);
                        if (aS && !bS) return -1; if (!aS && bS) return 1; return a.fullName.localeCompare(b.fullName, 'he');
                      }).map(p => (
                        <button key={p.id} disabled={nP || isSaving} onClick={() => toggleSoldierInSlot(nightKey, p.id, day, 'night')} className={clsx("w-full text-right p-1.5 rounded-lg border transition-all text-[11px] font-bold flex justify-between items-center group", sNP.includes(p.id) ? "bg-slate-800 dark:bg-slate-700 border-zinc-900 dark:border-zinc-600 text-white shadow-sm" : "bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:border-slate-300 dark:hover:border-slate-500", (nP || isSaving) && "cursor-not-allowed border-transparent shadow-none")}><span className="truncate">{p.fullName}</span>{sNP.includes(p.id) && !nP && <X size={10} className="opacity-50" />}</button>
                      ))}
                    </div>
                  </div>
                </div>
