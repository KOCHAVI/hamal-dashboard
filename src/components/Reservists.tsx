...
import { Sun, Moon, Save, X, AlertTriangle, Clock, Shield, ChevronRight, ChevronLeft, Calendar, Filter, Search, Info } from 'lucide-react';
...
export const Reservists = () => {
  ...
  const myReservists = useMemo(() => personnel.filter(p => {
    ...
    return isGuest;
  }), [personnel, isAdmin, isGuest, activeUser, selectedTeamId, searchQuery]);
  ...
  return (
    <div className="h-full flex flex-col bg-transparent text-right transition-colors duration-200" dir="rtl">
      <div className="p-4 lg:p-8 border-b border-zinc-200/50 dark:border-zinc-800/50 bg-white/40 dark:bg-zinc-900/20 backdrop-blur-xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 transition-colors">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs lg:text-sm text-zinc-500 dark:text-zinc-400 mt-1 font-medium italic">{isGuest ? 'צפייה בלבד' : 'מציג אנשי מילואים בלבד'}</p>
            </div>
          </div>
          ...
          {isStaff && isAdmin && (
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                <input type="text" placeholder="חיפוש חייל..." className="bg-white/50 dark:bg-zinc-800/50 border border-zinc-200/50 dark:border-zinc-700/50 text-zinc-900 dark:text-white text-sm rounded-2xl outline-none p-2.5 pr-9 w-48 focus:ring-2 focus:ring-indigo-500/20" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        ...
        {isStaff && (
          <button onClick={handleSave} disabled={isSaving || !hasChanges} className={clsx("w-full lg:w-auto flex items-center justify-center gap-2 px-8 py-3 rounded-2xl font-bold transition-all shadow-lg active:scale-95", (isSaving || !hasChanges) ? "bg-zinc-400 dark:bg-zinc-800 cursor-not-allowed opacity-50 text-zinc-100" : "bg-indigo-600 hover:bg-indigo-700 text-white")}>
            {isSaving ? <Clock className="animate-spin" size={20} /> : <Save size={20} />} שמור סידור מילואים
          </button>
        )}
      </div>
                ...
                <div key={dayIdx} className="flex flex-col min-h-[500px] bg-zinc-50/30 dark:bg-zinc-900/10">
                  <div className={clsx("p-3 flex-1 flex flex-col transition-all", dP && "opacity-40 grayscale bg-zinc-200/50 dark:bg-zinc-800/50")}>
                    ...
                    <div className="flex-1 space-y-1">
                      {(isGuest ? personnel.filter(p => sDP.includes(p.id)) : myReservists)
                        .sort((a,b) => {
                        ...
                        <button
                          ...
                          className={clsx(
                            ...
                            isGuest && !sDP.includes(p.id) && "hidden",
                            isGuest && "cursor-default pointer-events-none"
                          )}
                        >
                          <span className="truncate">{p.fullName}</span>
                          {sDP.includes(p.id) && !isGuest && !dP && <X size={10} className="opacity-50" />}
                        </button>
                      ))}
                    </div>
                  </div>
                  <div className={clsx("p-3 flex-1 flex flex-col border-t border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 transition-all", nP && "opacity-40 grayscale bg-zinc-300/50 dark:bg-zinc-800/50")}>
                    ...
                    <div className="flex-1 space-y-1">
                      {(isGuest ? personnel.filter(p => sNP.includes(p.id)) : myReservists)
                        .sort((a,b) => {
                        ...
                        <button
                          ...
                          className={clsx(
                            ...
                            isGuest && !sNP.includes(p.id) && "hidden",
                            isGuest && "cursor-default pointer-events-none"
                          )}
                        >
                          <span className="truncate">{p.fullName}</span>
                          {sNP.includes(p.id) && !isGuest && !nP && <X size={10} className="opacity-50" />}
                        </button>
                      ))}
                    </div>
                  </div>
