...
export const Management = () => {
  ...
  const handleAddSoldier = async (e: React.FormEvent) => {
    ...
    await addPersonnel(soldierData);
    setSoldierData({ full_name: '', team_id: '', phone_number: '', is_reservist: false, is_abroad: false });
  };

  const isAdmin = activeUser?.isAdmin;
  const isHoTForSelectedTeam = activeUser && (
    isAdmin || 
    (activeUser.isHoT && teams.find(t => t.id === soldierData.team_id)?.leaderId === activeUser.id)
  );

  return (
    <div className="h-full overflow-y-auto bg-zinc-50 dark:bg-zinc-950 p-8 space-y-8 text-right transition-colors duration-200" dir="rtl">
      <h2 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tight">ניהול מערכת</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm lg:col-span-2 transition-colors">
          <div className="flex items-center gap-2 mb-4">
            <UserPlus className="text-indigo-500" size={20} />
            <h3 className="text-lg font-bold dark:text-white">הוספת חייל לצוות</h3>
          </div>
          <form onSubmit={handleAddSoldier} className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input 
              placeholder="שם החייל"
              className="p-3 border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all"
            ...
            <input 
              ...
              className="p-3 border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 text-left transition-all"
              dir="ltr"
            ...
            <div className="flex flex-wrap items-center gap-6 px-2">
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="isSoldierReservist" className="text-sm font-bold text-zinc-600 dark:text-zinc-400">חייל מילואים</label>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="isSoldierAbroad" className="text-sm font-bold text-purple-600">נמצא בחו"ל</label>
              </div>
            </div>
            ...
            <button 
              ...
              className={clsx(
                "py-3 rounded-xl font-bold text-white shadow-md transition-all md:col-span-4",
            ...
            >
              {isHoTForSelectedTeam ? "שמור חייל" : "אין הרשאה לצוות זה"}
            </button>
          </form>
        </div>
